...
- `min_queries`: minimum number of distinct search queries the searcher must
  run before declaring done (default: `4`)
...
- `queries`: optional list of search prompts; when omitted, derive them from
  `schema.yml` description + `README.md` (still must produce ≥ `min_queries`
  distinct queries)

### Ensures

- `candidates`: `workspace/discover/candidates.json` — an array of
...
- every candidate's `entity_id` follows `<company-slug>-<YYYY-MM-DD>` (lowercase,
  hyphenated company name + date of announcement)
- every candidate's `fields` object includes at least: `company`,
...
- a candidate without at least one evidence URL is dropped, not saved with `null`
...
- never include candidates where the company is not identifiable (e.g.
  "a major US retailer" with no name)
- announcements older than `freshness_days` are excluded — the curate refresh
  pass handles older entities, not discover

### Services

- `tool-picker`: inspects `mcp__pay__list_tools` and the schema/README;
  ensures `chosen_tools` (a ranked list — `serper_news` and `exa_search` are
  the obvious primaries; `twitter_search` and `reddit_search` are
  corroborators); ensures `workspace/tool-picker/chosen_tools.json`
- `query-planner`: derives ≥ `min_queries` distinct search queries spanning
  news, semantic web, and social. Examples:
  - `"layoffs 2026"` + `"announces layoffs <last-week-date>"` (serper_news)
  - `"workforce reduction announcement <month>"` (exa_search semantic)
  - `"company layoffs site:sec.gov 8-K"` (semantic, surfaces SEC filings)
  - `"layoffs" lang:en since:<freshness_days>` (twitter_search for official posts)
  - subreddit:layoffs OR subreddit:cscareerquestions `"laid off"` `"announced"` (reddit_search)
  ensures `workspace/query-planner/queries.json`
- `searcher`: invokes `mcp__pay__pay_tool` with the chosen tools **once per
  query**; ensures `raw_results[]` keyed by query
- `extractor`: maps `raw_results[]` into candidate rows shaped to
  `schema.yml`, populating every field the result directly evidences and
  tagging `authority_tier`; ensures `candidates_unfiltered[]`
- `dedup`: drops candidates whose `entity_id` already exists according to
  `mcp__frame-layoffs-2026__query`, dedupes across queries (same company +
  same date), and merges multi-source evidence onto a single candidate so
  the curate run sees the strongest tier; ensures `candidates`
...
- when the budget is half spent and `serper_news` is producing more
  candidates per dollar than `exa_search`, drop the semantic queries and
  spend the remaining budget on news
...
- when a single announcement spawns hits across news + Twitter + Reddit,
  merge them onto one candidate and keep the highest `authority_tier` (lower
  number = higher authority); store all URLs in `evidence_urls`
...
- when `reddit_search` surfaces an unconfirmed rumor (employees saying "I
  just got laid off" but no company statement and no news coverage), tag it
  `authority_tier: 5` and `confidence ≤ 0.3` — don't drop, but flag for
...
- when `twitter_search` returns a CEO post that names a number, tag
  ...
  statements, even if not on the company blog)
