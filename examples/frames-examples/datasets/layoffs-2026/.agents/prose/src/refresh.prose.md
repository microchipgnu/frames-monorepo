...
- when an official_statement_url goes dead but the underlying claim is
  ...
  `attach_evidence` of the news source over deprecating — losing one
  citation shouldn't lose a true fact

### Tools

- `mcp:frame-layoffs-2026`: required for fact enumeration and write-back
  (`query`, `deprecate_fact`, `attach_evidence`).
