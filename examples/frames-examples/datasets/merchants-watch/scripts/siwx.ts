...
// only implement eip191 here because mppscan only advertises eip191.
...
// Build the canonical EIP-4361 / SIWE message that gets signed. The format
// mppscan accepts matches the SIWE v1 spec, with domain/address/statement/
// uri/version/chainId/nonce/issuedAt etc.
...
export class SiwxClient {
  ...
  async fetch(url: string, init: RequestInit = {}): Promise<Response> {
    ...
    // Inspect the 402 — if it has a SIWX challenge, handle it.
    ...
    // Retry with the signed Authorization header.
...
/**
 * Convenience: return a SiwxClient if MPPSCAN_WALLET_PRIVATE_KEY is set,
 * else null. Lets the caller fall back to a cached snapshot.
 */
