...
async function fetchPage(
  ...
  while (true) {
    ...
    // Give up on this page but signal the caller so we don't fail the whole tick.
    console.error(
...
async function main() {
  ...
  // Group by canonical host (some services may share a host, take first)
  const byHost = new Map<string, AgenticStaging>();
  for (const s of all) {
    const host = canonicalHost(s.domain ?? s.providerUrl ?? null);
    if (!host) continue;
    if (byHost.has(host)) continue;

    const minU = parseUsd(s.priceSummary?.minAmount);
    const maxU = parseUsd(s.priceSummary?.maxAmount);
  ...
  console.error("✓ wrote staging/agentic-market.json");
}
