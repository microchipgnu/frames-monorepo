...
//
// pay.sh provides MPP discoverability + a normalized category taxonomy + a
// price range per provider. Used as the canonical category source in merge.
...
async function main() {
  ...
  while (true) {
    ...
    throw new Error(`pay.sh: HTTP ${res.status}`);
  }

  const byHost = new Map<string, PayshMerchant>();
  for (const p of providers) {
    ...
    byHost.set(host, {
      ...
      min_price_usd: typeof p.min_price_usd === "number" ? p.min_price_usd : null,
      max_price_usd: typeof p.max_price_usd === "number" ? p.max_price_usd : null,
